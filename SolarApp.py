...
with st.form("add_appliance_form"):
    ...
    if add_btn and selected_app != "-- เลือกเครื่องใช้ไฟฟ้า --":
        ...
        st.session_state.calculated = False # เมื่อเพิ่มของใหม่ ให้เคลียร์สถานะกดคำนวณ

# แสดงตารางรายการที่กดเพิ่มเข้ามาแล้วพร้อมฟังก์ชัน "ลบรายบรรทัด"
if st.session_state.load_items:
    st.markdown("#### 📋 รายการเครื่องใช้ไฟฟ้าที่เลือกไว้")
    
    # สร้างคอลัมน์แสดงผลบนเว็บแอป
    for index, item in enumerate(st.session_state.load_items):
        col_idx, col_name, col_detail, col_del = st.columns([1, 4, 6, 2])
        
        with col_idx:
            # ปรับลำดับให้เริ่มจากลำดับที่ 1 
        ...
        with col_detail:
            # ✨ จุดแก้ไข: เปลี่ยนจาก item['จำนวน'] เป็น item['จำนวน (เครื่อง)'] เพื่อให้ดึงข้อมูลคีย์หลักได้ถูกต้อง
        ...
        with col_del:
            # สร้างปุ่มลบแยกชิ้นเป็นรายบุคคล
            if st.button(f"❌ ลบชิ้นนี้", key=f"del_{index}"):
                st.session_state.pop(index) if hasattr(st.session_state, 'pop') else st.session_state.load_items.pop(index)
                st.session_state.calculated = False
                st.rerun()
...
if st.session_state.calculated and st.session_state.load_items:
    ...
    # 2.1: ปริมาณไฟฟ้าที่คาดว่าผลิตได้จริง (หน่วย / เดือน)
    ...
    # 2.2: มูลค่าไฟฟ้าที่ประหยัดได้ต่อเดือน (บาท / เดือน)
    ...
    # 2.3: ราคาขายสุทธิยกมาจากตารางเงื่อนไข (บาท)
    ...
    # 2.4: ระยะเวลาคืนทุนของระบบโดยประมาณ (ปี)
